...
You will love it.`)
	...
	is.Equal(greetInputObject.Fields[0].Type.TypeID, "github.com/sbward/oto/testdata/services.Page")
	is.Equal(greetInputObject.Fields[0].Type.IsObject, true)
	is.Equal(greetInputObject.Fields[0].Type.Multiple, false)
	is.Equal(greetInputObject.Fields[0].Type.Package, "github.com/sbward/oto/testdata/services")
	...
	is.Equal(greetOutputObject.Fields[0].Type.TypeID, "github.com/sbward/oto/parser/testdata/services/pleasantries.Greeting")
	...
	is.Equal(strangeInputObj.Embeds[0].NameJSON, "InfiniteLoop")
	is.Equal(strangeInputObj.Embeds[0].Embedded, true)
	is.Equal(len(strangeInputObj.Fields), 4)
	is.Equal(len(strangeInputObj.Embeds), 1)
	...
	is.Equal(welcomeInputObject.Fields[0].NameJSON, "recipients")       // changed by json tag
	...
	is.Equal(welcomeInputObject.Fields[1].NameJSON, "Name")
	...
	for i := range def.Objects {
		...
		case "Page":
			is.Equal(def.Objects[i].TypeID, "github.com/sbward/oto/testdata/services.Page")
